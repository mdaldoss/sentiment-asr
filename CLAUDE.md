...
| Hume Octave TTS | `pip install hume`, auth via `HUME_API_KEY`, client is `AsyncHumeClient` (`hume.tts.synthesize_json`). Free tier 10K chars/month. `description` field (Octave 1 only) sets delivery **independently of the transcript** — the control Cartesia's `emotion` param lacks. `speed` range 0.5–2.0 (not Cartesia's 0.6–1.5). Output: base64 `audio` field, requested `format` (wav/mp3/pcm), 48kHz. **Confirmed working** (`results/hume_probe.json`): F0 span 126.7 Hz with `description` vs 32.3 Hz without, on the identical text/voice — real differentiation Cartesia didn't show |
