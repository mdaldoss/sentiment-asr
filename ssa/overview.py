...
<h3>4. Synthetic emotional speech: one vendor failed, another worked</h3>
...
<ol class="improve" start="5">
  <li><strong>76 generated clips were never evaluated.</strong> The synthetic incongruence
  set contains 52 clips where words and tone deliberately disagree &mdash; more than the
  36 in the human set that carry the headline prosody metric. It was set aside once the
  audio was found to be flat, but running it costs one command, and a poor score on
  deliberately flat audio is itself a publishable control.</li>
...
<div class="tier">
<h3>Tier 3 &mdash; making it a convincing demonstration <span class="pill">the visible gap</span></h3>
<ol class="improve" start="9">
  <li><strong>You cannot speak into it.</strong> There is a command-line entry point that
  classifies a file, but no way to record and get an answer back. For demonstrating that a
  model understands emotion, this is the most conspicuous absence &mdash; a reviewer's own
  voice, scored live, is worth more than any table.</li>

  <li><strong>Show the valence/arousal plane, not just a label.</strong> The system already
  computes a two-dimensional emotional reading internally and collapses it to one of three
  words. A live needle moving in that plane as someone speaks demonstrates "it is
  listening" far better than a metric; the data is already there and unused in the interface.</li>

  <li><strong>Show both backends disagreeing, live.</strong> Given that they disagree on
  real voices, letting a reviewer hear their own speech scored two ways turns the
  project's central finding into something they can feel rather than read.</li>
  ...
  smooths everything toward neutral &mdash; much more informative about <em>understanding</em>.</li>
</ol>
</div>
...
def build_overview() -> str:
    ...
    body = f"""
<header>
  <h1>Speech Sentiment Analyzer &mdash; overview</h1>
  <p class="tagline">What the field can do, what we built, what we found, and what would
  make it better. &mdash; <a href="../index.html">index</a> &middot;
  <a href="index.html">full results dashboard</a></p>
</header>
{render_the_question()}
...
"""
    return _page("Speech Sentiment Analyzer — Overview", body).replace(
